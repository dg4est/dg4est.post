/**
 * \file    analysis_pod_traditional.c
 * \ingroup utils_group
 * \author  akirby
 *
 * \brief   Computes the Proper Orthogonal Decomposition using the traditional
...
void pod_traditional(inputs_t *data,char standalone){
    ...
    for (ex = data->ext_sind; ex <= data->ext_eind; ex+=data->ext_ind_skip) {
        ...
        /* ====================== */
        compute_extract_pod_trad(analysis,mean,data,ex);

        /* write tec POD energy files */
        output_POD_energies(analysis,data,ex,analysis->npts);
        ...
        if(data->pod_plot_ROM || data->pod_plot_TVC){
            for (i = data->plot_tec_sind; i <= data->plot_tec_eind; i+=data->plot_tec_ind_skip) {
                ...
                compute_pod_trad_reconstruction(analysis,mean,data,ex,i,filename);
...
void compute_extract_pod_trad(extracts_t *analysis,extracts_t *means,inputs_t *data,int ex){
    Real flucTime=0.0;
    Real covaTime=0.0;
    Real eignTime=0.0;
    char *filename;
...
#endif
    for (step=0,i = data->file_sind; i <= data->file_eind; i+=data->file_ind_skip,step++) {
        if(!form_file_name(data,ex,i,&filename,DISPLAY_OFF,"FLUCTUATION")) continue;

        /* display status */
        Real progress = (step/(Real) analysis->nStep)*100.0;
        printf("\r[COVARIANCE] Calculation in progress (%d vars): %.1f%%",analysis->pod.npod,progress);

        TIMER(flucTime,compute_trad_fluctuations(analysis,means,filename););
        TIMER(covaTime,compute_trad_covariance(analysis,analysis->oneOnStepMone););
    ...
    /* ========================== */
    TIMER(eignTime,compute_trad_eigenproblem(analysis););
...
void compute_pod_trad_reconstruction(extracts_t *analysis,extracts_t *means,inputs_t *data,
                                int ex,int index,char *filename){
    /* re-compute solution fluctuation */
    compute_trad_fluctuations(analysis,means,filename);

    /* reconstruct flow solution */
    compute_trad_reconstruction(analysis,data,ex,index);
}

void compute_trad_fluctuations(extracts_t *analysis,extracts_t *means,char *file_name){
...
void compute_trad_reconstruction(extracts_t *analysis,inputs_t *data,int iextract,int istep){
    ...
    if(data->pod_nmode < 1 || data->pod_nmode > analysis->npts){
        data->pod_nmode = analysis->npts;
    }
    ...
    pod->R_a_p.newmalloc(NPTS*NPOD);
    pod->TVC.newmalloc(NMODES*NSTEP*NPOD,0.0);

    for (i = 0; i < NPOD; i++) {
        const Real * const R      = &pod->R_tot[NPTS*NPTS*i];
        const Real * const eigvec = &pod->eig[NPTS*i];
        Real * const Rap          = &pod->R_a_p[NPTS*i];
        ...
        POD_trad_reconstruct_solution_(&iextract,&istep,&pod_var,&nfield,
...
void compute_trad_covariance(extracts_t *analysis,Real oneOnStepMone){
    ...
    /* allocate memory (1st pass) */
    pod->R_tot.newmalloc(NPTS*NPTS*NPOD,0.0);

    /* =========================== */
    /* calculate covariance matrix */
    /* =========================== */
    for (i = 0; i < NPOD; i++) {
        Real * const R = &pod->R_tot[NPTS*NPTS*i];
        const int pod_var = pod->pod_vars[i]+FBASE; // add FORTRAN Base-1 index

        POD_trad_covariance_(&pod_var,&nfield,&NPTS,&oneOnStepMone,dU,R);
    }
}
...
void compute_trad_eigenproblem(extracts_t *analysis){
    ...
    /* allocate memory (1st pass) */
    pod->eig.newmalloc(NPTS*NPOD,0.0);

    /* ==================================== */
    /* calculation eigenvalues and vectors  */
    /* ==================================== */
    for (i = 0; i < NPOD; i++) {
        Real *R = &pod->R_tot[NPTS*NPTS*i]; // get address to POD variable
        Real *eigval = &pod->eig[NPTS*i]; // get address to POD variable

        int pod_var = pod->pod_vars[i];
        Real *energy_total = &pod->pod_total_energy[pod_var];

        POD_trad_solve_(&NPTS,R,eigval,energy_total);
